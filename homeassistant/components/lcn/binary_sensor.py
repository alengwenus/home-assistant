...
def create_lcn_binary_sensor_entity(hass, entity_config, config_entry):
    ...
    if entity_config[CONF_DOMAIN_DATA][CONF_SOURCE] in SETPOINTS:
        entity = LcnRegulatorLockSensor(entity_config, device_connection)
    elif entity_config[CONF_DOMAIN_DATA][CONF_SOURCE] in BINSENSOR_PORTS:
        entity = LcnBinarySensor(entity_config, device_connection)
    else:  # in KEY
        entity = LcnLockKeysSensor(entity_config, device_connection)
    return entity

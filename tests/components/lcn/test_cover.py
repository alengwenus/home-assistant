...
from pypck.lcn_defs import MotorPort, MotorReverseTime, MotorStateModifier, OutputPort
...
async def test_setup_lcn_cover(activate_status_request_handler, hass, entry):
    ...
    calls = [
        ...
        call(OutputPort.OUTPUTDOWN),
        call(MotorPort.MOTOR1),
    ]
...
async def test_outputs_open(control_motors_outputs, hass, entry):
...
@patch.object(MockModuleConnection, "control_motors_outputs")
async def test_outputs_close(control_motors_outputs, hass, entry):
...
@patch.object(MockModuleConnection, "control_motors_outputs")
async def test_outputs_stop(control_motors_outputs, hass, entry):
...
async def test_relays_open(control_motors_relays, hass, entry):
...
@patch.object(MockModuleConnection, "control_motors_relays")
async def test_relays_close(control_motors_relays, hass, entry):
...
@patch.object(MockModuleConnection, "control_motors_relays")
async def test_relays_stop(control_motors_relays, hass, entry):
...
async def test_unload_config_entry(hass, entry):
    ...
    await setup_platform(hass, entry, DOMAIN_COVER)
    await hass.config_entries.async_forward_entry_unload(entry, DOMAIN_COVER)
    assert not hass.states.get("cover.cover_outputs")
    assert not hass.states.get("cover.cover_relays")
